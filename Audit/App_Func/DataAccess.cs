    ...
    public class DataAccess
    {
        ...
        public static DataResponse BM1(XElement request)
        {
            ...
            try
            {
                ...
                cmd.CommandText = "SELECT COUNT(BM.ID) " +
                    ...
                    //"OR (:V_USER_TYPE IN ('BRANCH_AUDITOR','HAK_USER') AND B.DEPARTMENT_ID = :V_DEPARTMENT AND (B.CREATED_BY = :V_USER_ID OR B.UPDATED_BY = :V_USER_ID " +
        ...
        public static DataResponse BM1UpdateCompletion(XElement request)
        {
            ...
            try
            {
                ...
                cmd.Parameters.Add(":P_REMOVED_AMOUNT", OracleDbType.Varchar2).Value = elem.Element("REMOVED_AMOUNT")?.Value == null || elem.Element("REMOVED_AMOUNT")?.Value == "0.00" ? null : elem.Element("REMOVED_AMOUNT")?.Value;
        ...
        public static DataResponse NM1(XElement request)
        {
            ...
            try
            {
                ...
                cmd.CommandText = "SELECT "+
                                ...
                                "INNER JOIN AUD_STAT.REF_AUDIT_YEAR RAY ON B.AUDIT_YEAR = RAY.YEAR_ID " +
                                "INNER JOIN AUD_STAT.REF_AUDIT_TYPE RAT ON B.AUDIT_TYPE = RAT.AUDIT_TYPE_ID " +
                                "WHERE NM.IS_ACTIVE = 1 AND B.IS_ACTIVE = 1 AND (:V_USER_TYPE != 'Branch_Auditor' OR(:V_USER_TYPE = 'Branch_Auditor' AND B.DEPARTMENT_ID = :V_DEPARTMENT)) AND (:V_FILTER_DEPARTMENT IS NULL OR (B.DEPARTMENT_ID = :V_FILTER_DEPARTMENT)) " +
                                "AND b.STATISTIC_PERIOD = :V_PERIOD AND(:V_SEARCH IS NULL OR UPPER(B.TOPIC_CODE) LIKE '%' || UPPER(:V_SEARCH) || '%' " +
