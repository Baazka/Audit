...
namespace Audit.Models
{
    ...
    public class BM2List
    {
        ...
        public BM2List SetXml(XElement xml)
        {
            if (xml != null)
            {
                ...
                if (xml.Element("CLAIM_C2_NONEXPIRED") != null)
                    CLAIM_C2_NONEXPIRED = Convert.ToDecimal(xml.Element("CLAIM_C2_NONEXPIRED").Value).ToString("#,0.00");
                if (xml.Element("CLAIM_C2_EXPIRED") != null)
                    CLAIM_C2_EXPIRED = Convert.ToDecimal(xml.Element("CLAIM_C2_EXPIRED").Value).ToString("#,0.00");
